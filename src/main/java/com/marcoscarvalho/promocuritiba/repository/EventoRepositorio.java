...
public interface EventoRepositorio extends CrudRepository<Evento, Long> {
	...
	@Query(value = "SELECT * FROM evento E WHERE exists (select 1 from data where data_inicio >= now())", nativeQuery = true)
	public List<Evento> findNextEvents();

	@Query(value = "SELECT * FROM evento E WHERE exists (select 1 from data where data_inicio >= ?1 )", nativeQuery = true)
	public List<Evento> findNextEvents(Calendar data);

	@Query(value = "SELECT * FROM evento E WHERE exists (select 1 from data where data_inicio between ?1 and ?2 )", nativeQuery = true)
	public List<Evento> findNextEvents(Calendar dataInicio, Calendar dataFim);
	
	@Cacheable("ultimaAtualizacao")
	@Query(value = "select max(data_alteracao) from evento", nativeQuery = true)
